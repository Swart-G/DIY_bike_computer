...
void SettingsScreen::drawRide(TFT_eSPI& tft, const SettingsStatus& status,
  ...
  row(tft, 169, "Auto pause",
      settings.autoPauseEnabled
          ? "On - " + String(settings.autoPauseDelayMs / 1000.0f, 1) + " s"
          : "Off");
  row(tft, 225, "Log interval",
      String(settings.logSampleIntervalMs / 1000.0f, 1) + " s");
}
...
void SettingsScreen::drawDisplay(TFT_eSPI& tft, const SettingsStatus& status) {
  ...
  row(tft, 57, "Theme", "Dark");
  row(tft, 113, "Screen timeout", "Never");
  row(tft, 169, "Ride page order", "Speed / Stats / Graph");
  row(tft, 225, "Status chips", "Compact");
}
...
void SettingsScreen::drawSystem(TFT_eSPI& tft, const SettingsStatus& status) {
  ...
  row(tft, 57, "SD card", status.sdText);
  row(tft, 113, "USB storage",
      status.rideActive
          ? "Locked during ride"
          : (status.usbActive ? "Active" : "Inactive"));
  row(tft, 169, "Time source", status.timeSource);
  row(tft, 225, "Firmware", app::FIRMWARE_VERSION);
  if (status.showRideLockNotice) {
    tft.fillRoundRect(69, 276, 342, 32, 10, WARNING);
    tft.setTextDatum(MC_DATUM);
    tft.setTextColor(BG, WARNING);
...
void SettingsScreen::drawWheel(TFT_eSPI& tft, const SettingsStatus& status,
  ...
  Components::header(tft, "Wheel circumference", status.header);
  tft.fillRect(160, 76, 160, 78, BG);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TEXT, BG);
  tft.drawString(String(settings.wheelCircumferenceM, 3), 240, 104, 6);
  tft.setTextColor(TEXT_MUTED, BG);
  tft.drawString("m", 240, 137, 2);
  tft.fillRect(92, 243, 296, 11, BG);
  tft.drawWideLine(94, 248, 386, 248, 4.0f, BORDER, BG);
  const int16_t marker = 94 + static_cast<int16_t>(
      constrain((settings.wheelCircumferenceM - 0.5f) / 3.0f, 0.0f, 1.0f) * 292.0f);
  ...
  tft.setTextDatum(ML_DATUM);
  tft.setTextColor(TEXT_MUTED, BG);
  tft.drawString("0.500", 94, 269, 1);
  tft.setTextDatum(MR_DATUM);
  tft.drawString("3.500", 386, 269, 1);
...
void SettingsScreen::drawStopThreshold(TFT_eSPI& tft, const SettingsStatus& status,
                                       const app::AppSettings& settings) {
  ui_exact::ExactScreenRenderer(tft).draw(
      ui_exact::ScreenId::SCREEN_76_SETTINGS_STOP_THRESHOLD);
  Components::header(tft, "Stop threshold", status.header);
  tft.fillRect(160, 76, 160, 78, BG);
  tft.setTextDatum(MC_DATUM);
  tft.setTextColor(TEXT, BG);
  tft.drawString(String(settings.stopThresholdKmh, 1), 240, 104, 6);
  tft.setTextColor(TEXT_MUTED, BG);
  tft.drawString("km/h", 240, 137, 2);
  tft.fillRect(92, 243, 296, 11, BG);
  tft.drawWideLine(94, 248, 386, 248, 4.0f, BORDER, BG);
  const int16_t marker = 94 + static_cast<int16_t>(
      constrain((settings.stopThresholdKmh - 0.5f) / 14.5f, 0.0f, 1.0f) * 292.0f);
  tft.drawWideLine(94, 248, marker, 248, 4.0f, ACCENT, BG);
  tft.fillCircle(marker, 248, 5, ACCENT);
  Components::button(tft, 74, 171, 86, 52, "-");
  Components::button(tft, 320, 171, 86, 52, "+", true);
  ...
  tft.setTextDatum(ML_DATUM);
  tft.setTextColor(TEXT_MUTED, BG);
  tft.drawString("0.5", 94, 269, 1);
  tft.setTextDatum(MR_DATUM);
  tft.drawString("15.0", 386, 269, 1);
  Components::button(tft, 150, 287, 180, 27, "Save", true);
}
...
void SettingsScreen::drawRgbLed(TFT_eSPI& tft,
  ...
  row(tft, 57, "Indicator",
      settings.rgbSpeedTrendEnabled ? "On" : "Off");
