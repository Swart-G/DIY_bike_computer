...
bool RideLogger::start(StorageManager& storage, const app::AppSettings& settings, const BatteryMonitor& battery, String& error) {
  ...
  sampleIndex_ = 0; sampleIntervalMs_ = settings.logSampleIntervalMs; bufferedCount_ = 0; bufferedHead_ = 0; loggingGap_ = false; active_ = true; lastGpsTimestampUtcMs_ = 0; gpsFixCount_ = 0; rideFormatVersion_ = app::RIDE_LOG_FORMAT_VERSION;
  ...
  if (!append(storage, path, "sample_index,ride_time_ms,state,speed_kmh,raw_speed_kmh,distance_m,avg_speed_kmh,max_speed_kmh,elapsed_time_ms,recording_time_ms,moving_time_ms,pause_time_ms,pulse_count,rejected_pulse_count,battery_voltage,battery_percent,timestamp_utc_ms,latitude,longitude,altitude_m,gps_accuracy_m,gps_speed_mps,gps_age_ms\n")) { error = "Cannot create samples"; active_ = false; return false; }
...
bool RideLogger::resume(StorageManager& storage, const RideRecoveryData& r, String& error) {
  ...
  char metaPath[64]; snprintf(metaPath, sizeof(metaPath), "%s/meta.json", r.rideFolder);
  StaticJsonDocument<768> meta;
  if (!storage.readJson(metaPath, meta, error)) return false;
  const uint8_t storedFormat = meta["format_version"] | 1;
  if (storedFormat != 1 && storedFormat != app::RIDE_LOG_FORMAT_VERSION) {
    error = "Unsupported recovery ride format";
    return false;
  }
  rideId_ = r.rideId; strncpy(folder_, r.rideFolder, sizeof(folder_)-1); folder_[sizeof(folder_)-1] = 0; sampleIndex_ = r.lastSavedSampleIndex; loggingGap_ = r.loggingGap; batteryStart_=r.batteryStartVoltage; batteryMin_=r.batteryMinVoltage; batteryMax_=r.batteryMaxVoltage; active_ = true; lastGpsTimestampUtcMs_ = 0; gpsFixCount_ = 0; rideFormatVersion_ = storedFormat;
...
bool RideLogger::logSample(StorageManager& storage, const RideStateMachine& ride, const SpeedCalculator& speed, const HallSensorSnapshot& sensor, const BatteryMonitor& battery, const phonegeo::LocationState& location, uint32_t nowMs) {
  ...
  lastSampleMs_ = nowMs; const RideStats& s = ride.stats(); char line[384];
  const float bv = battery.voltage(); if (bv > .1f) { if (!batteryMin_ || bv < batteryMin_) batteryMin_ = bv; if (bv > batteryMax_) batteryMax_ = bv; }
  const int prefixLength = snprintf(line, sizeof(line), "%lu,%llu,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%llu,%llu,%llu,%llu,%lu,%lu,%.3f,%u", static_cast<unsigned long>(sampleIndex_++), static_cast<unsigned long long>(s.elapsedMs), ride.stateText().c_str(), speed.currentKmh(), speed.rawKmh(), s.distanceM, s.averageMovingSpeedKmh, s.maxSpeedKmh, static_cast<unsigned long long>(s.elapsedMs), static_cast<unsigned long long>(s.recordingMs), static_cast<unsigned long long>(s.movingMs), static_cast<unsigned long long>(s.pauseMs), static_cast<unsigned long>(s.pulseCount), static_cast<unsigned long>(sensor.rejectedPulseCount), bv, battery.percent());
  if (prefixLength <= 0 || static_cast<size_t>(prefixLength) >= sizeof(line)) {
    loggingGap_ = true;
    return false;
  }
  if (rideFormatVersion_ == 1) {
    strlcpy(line + prefixLength, "\n", sizeof(line) - prefixLength);
  } else {
    const phonegeo::LocationFix& fix = location.fix;
    const bool freshFix = fix.rideId == rideId_ && fix.fresh(nowMs);
    if (freshFix) {
      char altitude[24] = {0}, accuracy[24] = {0}, gpsSpeed[24] = {0};
      if ((fix.flags & phonegeo::FixFlags::HasAltitude) != 0) snprintf(altitude, sizeof(altitude), "%.3f", fix.altitudeM());
      if ((fix.flags & phonegeo::FixFlags::HasAccuracy) != 0) snprintf(accuracy, sizeof(accuracy), "%.3f", fix.accuracyM());
      if ((fix.flags & phonegeo::FixFlags::HasSpeed) != 0) snprintf(gpsSpeed, sizeof(gpsSpeed), "%.3f", fix.speedMps());
      const size_t remaining = sizeof(line) - prefixLength;
      const int suffixLength = snprintf(line + prefixLength, remaining,
               ",%llu,%.7f,%.7f,%s,%s,%s,%lu\n",
               static_cast<unsigned long long>(fix.timestampUtcMs),
               fix.latitude(), fix.longitude(), altitude, accuracy, gpsSpeed,
               static_cast<unsigned long>(nowMs - fix.receivedAtMs));
      if (suffixLength <= 0 || static_cast<size_t>(suffixLength) >= remaining) {
        loggingGap_ = true;
        return false;
      }
      if (lastGpsTimestampUtcMs_ != fix.timestampUtcMs) {
        lastGpsTimestampUtcMs_ = fix.timestampUtcMs;
        ++gpsFixCount_;
      }
    } else {
      strlcpy(line + prefixLength, ",,,,,,,\n", sizeof(line) - prefixLength);
